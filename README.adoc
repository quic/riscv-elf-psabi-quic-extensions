...
```shell
git clone --recurse-submodules https://github.com/qualcomm/riscv-elf-psabi-quic-extensions.git
```

All in one single line:

```shell
git clone --recurse-submodules https://github.com/qualcomm/riscv-elf-psabi-quic-extensions.git && cd riscv-elf-psabi-quic-extensions && git submodule update --init --recursive
